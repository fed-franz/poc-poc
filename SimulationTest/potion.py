...
def createBlockchain(nodesNumber, maliciousNumber):
    ...
    os.system("docker exec -t node1 /btcbin/bitcoind-f3-tp -regtest -debug=net -daemon")
    ...
    for i in range(2, int(nodesNumber)+1):
        ...
        os.system("docker exec -t node" + str(i) + " /btcbin/bitcoind-f3-tp -regtest -debug=net -daemon")

    for i in range(int(nodesNumber)+1, int(nodesNumber)+1+int(maliciousNumber)):
        os.system("docker run -it -d --name node" + str(i) + " ubuntu /bin/bash")
        os.system("docker cp ../btcbin/ node" + str(i) + ":/")
        os.system("docker exec -t node" + str(i) + " /btcbin/bitcoind-f3-tp -malicious -regtest -debug=net -daemon")
    ...
    os.system("docker exec -t nodeMonitor /btcbin/bitcoind-f3-tp -regtest -pocmon -debug=net -daemon")
    time.sleep(1)

    os.system("docker run -it -d --name nodeMonitor2 ubuntu /bin/bash")
    os.system("docker cp ../btcbin/ nodeMonitor2:/")
    os.system("docker exec -t nodeMonitor2 /btcbin/bitcoind-f3-tp -regtest -pocmon -debug=net -daemon")
    time.sleep(1)
    
    os.system("docker run -it -d --name nodeMonitor3 ubuntu /bin/bash")
    os.system("docker cp ../btcbin/ nodeMonitor3:/")
    os.system("docker exec -t nodeMonitor3 /btcbin/bitcoind-f3-tp -regtest -pocmon -debug=net -daemon")
    time.sleep(1)
    
    os.system("docker run -it -d --name nodeMonitor4 ubuntu /bin/bash")
    os.system("docker cp ../btcbin/ nodeMonitor4:/")
    os.system("docker exec -t nodeMonitor4 /btcbin/bitcoind-f3-tp -regtest -pocmon -debug=net -daemon")
    time.sleep(5)
